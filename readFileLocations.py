        ...
        for path in save.filePaths:
            ...
            if oldHash != pathHash:
                ...
                if os.path.isfile(path):
                    ...
                    if (
                        fileSize > 100000000
                    ):  # files above 100MB, try zpaq level 5 compression
                        if output:
                            print("Zip is too big, trying zpaq level 5 compression")
                        os.remove(os.path.join("saves", f"{save.appName}.zip"))
                        zpaq_path = os.path.join("saves", f"{save.appName}.zpaq")
                        subprocess.run(
                            ["zpaq", "a", zpaq_path, path, "-m5"], check=True
                        )
                        zpaqSize = os.path.getsize(zpaq_path)
                        if zpaqSize > 100000000:
                            if output:
                                print(
                                    "zpaq still too big, using folder directory instead"
                                )
                            os.remove(zpaq_path)
                            shutil.copytree(
                                path,
